...
Use the `terminal` preset when the project should start with the theme and every component:

```bash
npx shadcn@latest init https://afterglow.thebuilder.dk/r/terminal.json
```

This installs the theme, interface components, terminal-specific components, and hooks. Add composed blocks separately after configuring the namespace.
